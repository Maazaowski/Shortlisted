...
- Choose the bullets most relevant to the posting's must-have skills and responsibilities. Prefer bullets with metrics. Eight to twelve bullets across all roles is typical. Include every role, with fewer bullets for less relevant ones. Projects only if they carry skills the roles do not.
...
Extract every role and project with every bullet you find. Do not merge, summarise, or drop bullets. Keep the candidate's wording. For each bullet, tag the technologies and skills it demonstrates in lowercase, and pull out its headline metric if it has one. Dates are YYYY-MM; use null when unknown. Skill groups come from the skills section if the resume has one, otherwise group the tagged skills sensibly.`;
